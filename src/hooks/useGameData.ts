 ...
 * 
 * Facilite l'utilisation du GameDataService dans les composants
 */
...
export const useThemeSearch = () => {
  ...
  const [themes, setThemes] = useState(gameDataService.getChampionThemes());
  ...
  useEffect(() => {
    if (searchQuery.trim() === '') {
      ...
      const results = gameDataService.searchThemes(searchQuery);
